...
with tabs[1]:
    ...
    st.markdown("""
        ...
        Upload the GTF file below or use a default remote download option.
    """)
    ...
    gtf_file = st.file_uploader("Upload GTF File", type=["gtf"], help="Upload the GTF annotation file (e.g., GENCODE).")
    ...
    if st.button("Annotate Results"):
        try:
            if gtf_file is None:
                st.error("Please upload a GTF file to proceed with annotation.")
            else:
                # Save uploaded file temporarily
                with tempfile.NamedTemporaryFile(delete=False, suffix=".gtf") as tf:
                    tf.write(gtf_file.read())
                    gtf_file_path = tf.name
                
                # Process the file
                process_files(input_file, gtf_file_path, output_prefix)
